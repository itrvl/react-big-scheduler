...
class ResourceEvents extends Component {
  ...
  render() {
    ...
    // resource view
    const { resourceTableWidth, resourceView, groupOnly } = this.props;
    ...
    resourceEvents.headerItems.forEach((headerItem, index) => {
      if (headerItem.count > 0 || headerItem.summary != undefined) {
        let isTop =
          config.summaryPos === SummaryPos.TopRight || config.summaryPos === SummaryPos.Top || config.summaryPos === SummaryPos.TopLeft;
        ...
        if (headerItem.summary != undefined) {
          ...
          let summary = (
            <Summary key={key} schedulerData={schedulerData} summary={headerItem.summary} left={left} width={width} top={top} />
          );
      ...
      <tr>
        <td>
          <div style={{ display: 'flex', flexDirection: 'row', marginBottom: 10 }}>
            <div style={{ width: groupOnly ? 'auto' : resourceTableWidth }}>{resourceView}</div>
            <div style={{ display: 'flex', flex: 1 }}>
              {connectDropTarget(
                <div ref={this.eventContainerRef} style={{ display: 'flex', flexDirection: 'row', flex: 1 }} className="event-container">
                  {eventList}
                </div>,
              )}
            </div>
