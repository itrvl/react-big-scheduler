...
class AgendaEventItem extends Component {
  ...
  render() {
    ...
    let content = (
      <EventItemPopover {...this.props} title={eventItem.title} startTime={eventItem.start} endTime={eventItem.end} statusColor={bgColor} />
    );
    ...
    return config.eventItemPopoverEnabled ? (
      <Popover placement="bottomLeft" content={content} trigger="hover" mouseEnterDelay={0}>
        <a
          className="day-event"
          onClick={() => {
